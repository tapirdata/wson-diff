...
export type ArrayLimiter = (have: AnyArray, wish: AnyArray) => number;
export type MdxOfs = [number, number];
export type KeyUse = MdxOfs[];
export interface KeyUses {
  [key: string]: KeyUse;
}
...
export class Modifier {
  ...
  public addPreLeg(leg: Leg): number {
    return this.legs.push(leg);
  }

  public setupLegs(): void {
    const { mdx } = this;
    const outLegs: Leg[] = _(this.legs)
      .filter((leg) => leg.haveMdx === mdx)
      .sortBy('haveOfs')
      .value();
    const inLegs: Leg[] = _(this.legs)
  ...
  public getDeletes(meModOff: number, cb: (pos: number, len: number) => void): void {
  ...
  public getInserts(meModOff: number, cb: (havePos: number, wishPos: number, len: number) => void): void {
    debug(
      'getInserts: mdx=%o meModOff=%o have=%o+%o wish=%o+%o',
      this.mdx,
      meModOff,
      this.haveBegin,
      this.haveLen,
  ...
  public getPatches(meModOff: number, cb: (havePos: number, wishPos: number, len: number) => void): void {
    debug(
      'getPatches: mdx=%o meModOff=%o have=%o+%o wish=%o+%o',
      this.mdx,
      meModOff,
      this.haveBegin,
      this.haveLen,
  ...
  public putMove(legId: number): number {
  ...
  public getMoves(meModOff: number, cb: (srcPos: number, dstPos: number, len: number, reverse: boolean) => void): void {
...
export class ArrayDiff {
  public state: State;
  public have: AnyArray;
  public wish: AnyArray;
  ...
  public setupModifiers(limit: number | ArrayLimiter | undefined): void {
    const { haveIdxer, wishIdxer } = this;

    const modifiers: Modifier[] = [];
    const wishKeyUses: KeyUses = {};

    const ad = this as ArrayDiff;
  ...
  public getModOffDiff(fromMdx: number, toMdx: number): number {
    let sum = 0;
    const { modifiers } = this;
    if (fromMdx < toMdx) {
      let idx = fromMdx;
  ...
  public getDelta(isRoot: boolean): string | null {
    if (this.modifiers.length === 0) {
      return null;
    } else {
      let delta = isRoot ? '|' : '';
  ...
  public debugModifiers(title: string): void {
    debug(title + ' modifiers:');
    this.modifiers.map(
      (modifier) => (
        debug(
          '  mdx=%o have=%o+%o wish=%o+%o, restBalance=%o',
          modifier.mdx,
          modifier.haveBegin,
          modifier.haveLen,
          modifier.wishBegin,
          modifier.wishLen,
          modifier.restBalance,
        ),
